...
export default function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  /* Entry animation sits on the wrapper, hover lift on the card itself. A
     filled CSS animation wins the cascade for the properties it animates, so
     sharing `transform` between the two would freeze the hover. */
  return (
    <div
      className="reveal"
    ...
    >
      <article
          className="group relative flex h-full flex-col overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg-raised)] transition-[border-color,box-shadow,transform] duration-300 hover:-translate-y-1.5 hover:border-[var(--border-strong)] hover:shadow-[var(--shadow)]"
        >
